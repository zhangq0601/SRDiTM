...
class Sampler(BaseSampler):
    ...
    def inference(self, in_path, out_path, mask_path=None, mask_back=True, bs=1, noise_repeat=False):
        ...
        '''
        def _process_per_image(im_lq_tensor, mask=None, chop_size=None, chop_stride=None):
            '''
            Input:
            ...
            if im_lq_tensor.shape[2] > self.chop_size or im_lq_tensor.shape[3] > self.chop_size:
                ...
                im_spliter = ImageSpliterTh(
                        im_lq_tensor,
                        self.chop_size if chop_size is None else chop_size,
                        stride=self.chop_stride if chop_stride is None else chop_stride,
                        sf=self.sf,
            ...
            for data in tqdm(dataloader):
                ...
                if micro_data['lq'].shape[0] > 0:
                    try:
                        ...
                        print(e)
                        print('Data Path:', micro_data['path'][0])  # OOM!
                        # sys.exit()
